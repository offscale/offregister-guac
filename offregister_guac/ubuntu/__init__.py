...
def configure_tomcat1(*args, **kwargs):
    ...
    upload_template(tomcat_users_local_filepath,
                           '{CATALINA_HOME}/conf/tomcat-users.xml'.format(CATALINA_HOME=CATALINA_HOME),
                           context={'ADMIN_USERNAME': kwargs.get('ADMIN_USERNAME', 'admin'),
    ...
    with cd(CATALINA_HOME):
        ...
        if run('id -u tomcat', warn_only=True, quiet=True).failed:
            ...
            sudo('useradd -s /bin/false -g tomcat -d {CATALINA_HOME} tomcat'.format(CATALINA_HOME=CATALINA_HOME))
        ...
        tomcat_service_local_filepath = kwargs.get(
            ...
            resource_filename(PKG_NAME,
                              path.join('_data', 'tomcat.service'))
        )
        upload_template(tomcat_service_local_filepath,
...
def install_deps():
    ...
    apt_depends('curl',
                ...
                'libpng12-dev', 'libossp-uuid-dev', 'libpango-1.0-0', 'libpango1.0-dev',
                'libssh2-1', 'libssh2-1-dev', 'libpulse-dev', 'libssl-dev', 'libvorbis-dev',
                'libwebp-dev', 'libfreerdp-dev',  # 'ffmpeg'
                'openjdk-8-jdk',  # <- client
                # From FreeRDP
                'build-essential', 'git-core', 'cmake', 'xsltproc', 'libssl-dev', 'libx11-dev', 'libxext-dev',
                ...
                'libavutil-dev', 'libavcodec-dev', 'libdirectfb-dev',
                # From libtelnet
    ...
    with cd('Downloads'):
        ...
        if not exists(libjpeg):
            run("curl -L 'https://downloads.sourceforge.net/project/libjpeg-turbo/{ver}/{pkg}' -o '{pkg}'".format(
        ...
        if not exists('{pkg}.tar.gz'.format(pkg=libvnc)):
            run('curl -OL https://github.com/LibVNC/libvncserver/archive/{pkg}.tar.gz'.format(pkg=libvnc))
        ...
        if not exists(libtelnet):
            run('curl -L https://github.com/seanmiddleditch/libtelnet/archive/master.tar.gz -o {pkg}.tar.gz'.format(
                pkg=libtelnet))
        ...
        if not exists(CATALINA_HOME):
            ...
            append('/etc/environment',
                   "export CATALINA_HOME='{CATALINA_HOME}'".format(CATALINA_HOME=CATALINA_HOME),
                   use_sudo=True)
            tomcat = 'apache-tomcat-9.0.1'
            tomcat_ver = tomcat.rpartition('-')[2]
            run('curl -OL http://mirror.intergrid.com.au/apache/tomcat/tomcat-{ver[0]}/v{ver}/bin/{pkg}.tar.gz'.format(
                pkg=tomcat, ver=tomcat_ver
            ))
            sudo('tar xf {pkg}.tar.gz -C {CATALINA_HOME} --strip-components=1'.format(pkg=tomcat,
...
def install_guac_client():
    with cd('Downloads'):
        if not cmd_avail('mvn'):
            ...
            run('curl -OL http://apache.melbourneitmirror.net/maven/maven-{ver[0]}/{ver}/binaries/{pkg}.tar.gz'.format(
                pkg=maven, ver=maven.rpartition('-')[0].rpartition('-')[2]
            ))
            d = '/opt/javalibs/maven'
            sudo('mkdir -p {d}'.format(d=d))
            sudo('tar xf {pkg}.tar.gz -C {d} --strip-components=1'.format(pkg=maven, d=d))
            with cd(d):
                sudo("ln -s '{d}/bin/mvn' '/usr/local/bin/$b'".format(d=d), shell_escape=False)
        guac_client = '0.9.13-incubating'
        if not exists('{pkg}.tar.gz'.format(pkg=guac_client)):
            run('curl -OL https://github.com/apache/incubator-guacamole-client/archive/{pkg}.tar.gz'.format(
                pkg=guac_client))
